...
test('Domain/Utility/Calculate - getDistanceOfPointToPoint', (t) => {
    ...
    each`
        ...
        ${[9, 9]}                                 | ${[1, 9]}                                 |   889560.63787495      |   878591.1714926899          |   879646.242669708
        ${[5.911760330200195, 51.97496770044958]} | ${[5.900301933288574, 51.97938231105512]} |     1365.4087816000642 |      925.6976780365701       |      927.9989469341483
        ${[4.8422, 45.7597]}                      | ${[2.3508, 48.8567]}                      |   441970.48063825216   |   392217.2577987035          |   392431.52894877753
        ${[-180, -90]}                            | ${[180, 90]}                              | 44755156.2689204       | 20015114.352186374           | 20003931.4586233
        ${[-180, 90]}                             | ${[180, -90]}                             | 44755156.2689204       | 20015114.352186374           | 20003931.4586233
        ${[-180, 0]}                              | ${[180, 0]}                               | 40030228.70437275      |        1.5604470998469443e-9 |        1.5569552749225338e-9
    `(({ a, b, ...calc }: any) => {
...
test('Domain/Utility/Calculate - getDistanceOfPointToLine', (t) => {
    ...
    each`
        ...
        -------------------------------------------|---------------------|-----------------------|-----------------------|----------
        ${[1, 1]}                                  | ${[[0, 0], [3, 3]]} |      0                |      0                |      0
        ${[2, 1]}                                  | ${[[0, 0], [3, 3]]} |  78626.794914751      |  78617.31500879859    |  78442.46603900735
        ${[1, 2]}                                  | ${[[0, 0], [3, 3]]} |  78626.794914751      |  78608.33580070207    |  78433.68568647825
        ${[5, 2]}                                  | ${[[0, 0], [9, 0]]} | 222390.1594687375     | 222390.1594687375     | 221149.45337244959
        ${[5, 2]}                                  | ${[[0, 0], [0, 9]]} | 555975.3986718437     | 555636.4985775814     | 556260.4424545397
        ...
        ${[5.909668207168579, 51.979065108032444]} | ${line}             |    341.51430750144857 |    327.53158580286527 |    327.7879564857658
    `(({ point, line, ...calc }: any) => {
        Object.keys(calc).forEach((key) => {
            const value = Number(calc[key]);
...
test('Domain/Utility/Calculate - getDistanceOfLineToLine', (t) => {
    ...
    each`
        ...
        ${[[1, 1], [20, 2]]} | ${[[0, 0], [30, 2]]} |  73965.86679042356 |  73965.72628462575 |  73555.60559277066
        ${[[2, 2], [4, 2]]}  | ${[[1, 1], [5, 1]]}  | 111195.07973436874 | 111195.07973436874 | 110575.0648144927
