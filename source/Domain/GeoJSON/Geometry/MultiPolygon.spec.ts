...
import { isMultiPolygon, isMultiPolygonCoordinates, isStrictMultiPolygon, isStrictMultiPolygonCoordinates } from './MultiPolygon';

describe('isMultiPolygonCoordinates', () => {
	test('accepts an array of polygon coordinate sets', () => {
		assert.ok(isMultiPolygonCoordinates([
			[[[0, 0], [1, 0], [1, 1], [0, 0]]],
			[[[2, 2], [3, 2], [3, 3], [2, 2]]],
		]));
...
describe('isStrictMultiPolygonCoordinates', () => {
	test('rejects out-of-range coordinates', () => {
		assert.ok(!isStrictMultiPolygonCoordinates([[[[-181, 0], [-181, 1], [0, 1], [0, 0], [-181, 0]]]]));
	});
});

describe('isMultiPolygon', () => {
	test('accepts a valid MultiPolygon', () => {
		assert.ok(isMultiPolygon({ type: 'MultiPolygon', coordinates: [[[[0, 0], [1, 0], [1, 1], [0, 0]]]] }));
