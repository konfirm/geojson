	...
	describe('karney', () => {
		...
		test('sub-millimetre shortline geodesics', () => {
			each`
				a                       | b                                                | s12
				------------------------|--------------------------------------------------|---
				${[0, 35.36284833173]}  | ${[0.000000008643945765, 35.36284832650837]}  | 0.0009760999388442188
				${[0, 3.215026142987]}  | ${[0.000000139248411094, 3.215027707832632]}   | 0.1737278999695807
				${[0, 83.608913171018]} | ${[0.000007497125466434, 83.60891429784064]}   | 0.15660299933829128
			`(({ a, b, s12 }: Improbability) => {
				assert.strictEqual(
					karney(a, b),
					Number(s12),
		...
		test('near-polar near-antipodal geodesics requiring bisection', () => {
			each`
				a                       | b                                                  | s12
				------------------------|----------------------------------------------------|---
				${[0, 82.045147306931]} | ${[179.9903970200043, 82.25908701585662]}  | 1753012.0559770009
				${[0, 81.118123796511]} | ${[179.9965245233924, 81.43574860292065]}   | 1948475.7898285992
				${[0, 84.67513518516]}  | ${[179.99666657227883, 85.32724243715317]}  | 1116645.2674368003
			`(({ a, b, s12 }: Improbability) => {
				assert.strictEqual(
					karney(a, b),
					Number(s12),
